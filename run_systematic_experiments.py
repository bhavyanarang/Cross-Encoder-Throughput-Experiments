...
def take_screenshot(experiment_name: str) -> str:
    ...
    try:
        ...
        subprocess.run(["screencapture", "-x", str(screenshot_path)], timeout=5)
...
def save_results_with_config(
    results: list, config: dict, output_file: Path, screenshot_path: str = ""
):
    ...
    with open(output_file, "w") as f:
        ...
        if successful:
            ...
            f.write("\n## Key Findings\n\n")
            f.write("| Metric | Value | Configuration |\n")
            f.write("|--------|-------|---------------|\n")
            f.write(
                f"| **Best Throughput** | {best_throughput['throughput_avg']:.1f} pairs/s | batch={best_throughput['batch_size']}, conc={best_throughput['concurrency']} |\n"
            )
            f.write(
                f"| **Best Latency** | {best_latency['latency_avg_ms']:.1f}ms | batch={best_latency['batch_size']}, conc={best_latency['concurrency']} |\n"
            )
...
def run_experiment(experiment_path: Path) -> dict:
    ...
    server_cmd = [python_exec, "ml_inference_server/main.py", "--experiment", str(experiment_path)]
...
def main():
    ...
    for experiment_path in experiments:
        ...
        if not result["success"]:
            logger.warning(
                f"Experiment {experiment_path.stem} failed: {result.get('error', 'Unknown')}"
            )
    ...
    for exp_name, result in results.items():
        status = (
            "✓ SUCCESS" if result["success"] else f"✗ FAILED: {result.get('error', 'Unknown')[:50]}"
        )
