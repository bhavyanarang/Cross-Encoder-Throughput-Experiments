...
class ModelInstanceConfig(BaseModel):
    ...
    backend: Literal["pytorch", "mps", "mlx", "onnx", "compiled"] = Field(
        default="mps", description="Backend implementation to use"
    )
    ...
    max_length: int | None = Field(
        default=None, description="Max sequence length for tokenization (None = use model default)"
    )
...
class ModelPoolConfig(BaseModel):
    ...
    instances: list[ModelInstanceConfig] = Field(
        default_factory=list, description="List of model instances to load"
    )
    routing_strategy: Literal[
        "round_robin", "least_busy", "first_available", "first_idle", "smart_idle"
    ] = Field(
        default="round_robin", description="Strategy for routing requests to model instances."
    )
...
class BatchingConfig(BaseModel):
    ...
    length_aware_batching: bool = Field(
        default=False, description="Sort pairs by length to reduce padding"
    )
...
class ScreenshotConfig(BaseModel):
    ...
    output_dir: str = Field(
        default="docs/experiments/screenshots", description="Directory for screenshots"
    )
...
class LegacyModelConfig(BaseModel):
    ...
    max_length: int | None = Field(default=None, description="Max sequence length for tokenization")
...
class ServerConfig(BaseModel):
    ...
    model_pool: ModelPoolConfig | None = Field(
        default=None, description="Multi-model pool configuration"
    )

    # Legacy single-model config (for backward compatibility)
    model: LegacyModelConfig | None = Field(
        default=None, description="Legacy single-model configuration"
    )
    ...
    def ensure_model_config(self) -> "ServerConfig":
        ...
        if self.model_pool is None and self.model is None:
            ...
            self.model = LegacyModelConfig(name="cross-encoder/ms-marco-MiniLM-L-6-v2")
        return self
    ...
    def get_model_pool_config(self) -> ModelPoolConfig:
        ...
        if self.model is not None:
            ...
            return ModelPoolConfig(instances=[instance], routing_strategy="round_robin")
