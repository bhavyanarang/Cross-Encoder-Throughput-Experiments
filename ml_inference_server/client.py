...
def benchmark(stub, all_pairs: list, batch_size: int, num_requests: int, concurrency: int = 1):
    ...
    logger.info(
        f"Starting benchmark: {num_requests} requests, concurrency={concurrency}, batch_size={batch_size}"
    )
    ...
    logger.info(
        f"{status.title()}: {elapsed:.2f}s | {total_pairs} pairs | {avg_throughput:.2f} pairs/s"
    )
...
def save_results_to_markdown(
    results: list,
    config: dict,
    output_file: str = "docs/experiment_results.md",
    partial: bool = False,
):
    ...
    with open(output_file, "w") as f:
        ...
        if batching_enabled:
            f.write(
                f" (max_batch={batching_config.get('max_batch_size', 'N/A')}, timeout={batching_config.get('timeout_ms', 'N/A')}ms)"
            )
        f.write("\n\n")
        f.write("**Model Type:** Cross-Encoder\n\n")
        ...
        for r in results:
            if "error" in r:
                f.write(
                    f"| {r['batch_size']} | {r['concurrency']} | ERROR | - | - | - | - | - | - |\n"
                )
            else:
                f.write(
        ...
        for i, r in enumerate(results, 1):
            if "error" in r:
                f.write(
                    f"### Config {i}: batch={r['batch_size']}, concurrency={r['concurrency']} - **ERROR**\n\n"
                )
                f.write(f"Error: {r.get('error', 'Unknown')}\n\n")
            else:
                f.write(
                    f"### Config {i}: batch={r['batch_size']}, concurrency={r['concurrency']}\n\n"
                )
        ...
        if successful:
            ...
            f.write("|--------|-------|--------|\n")
            f.write(
                f"| Best Throughput | {best_throughput['throughput_avg']:.2f} p/s | batch={best_throughput['batch_size']}, conc={best_throughput['concurrency']} |\n"
            )
            f.write(
                f"| Best Latency | {best_latency['latency_avg_ms']:.2f}ms | batch={best_latency['batch_size']}, conc={best_latency['concurrency']} |\n"
            )
    ...
    with open(combined_file, "a") as f:
        ...
        f.write(
            f"**Backend:** `{backend}` | **Device:** `{device}` | **Dynamic Batching:** `{batching_enabled}`\n\n"
        )
...
def run_experiments(stub, config, pairs: list):
    ...
    for batch_size in batch_sizes:
        for concurrency in concurrency_levels:
            if _interrupted:
                logger.warning("Skipping remaining experiments due to interrupt")
                break

            current += 1
            logger.info(f"\n{'='*60}")
            logger.info(
                f"Experiment {current}/{total_experiments}: batch_size={batch_size}, concurrency={concurrency}"
            )
            logger.info(f"{'='*60}")
            try:
...
def main():
    ...
    parser.add_argument(
        "--dataset-size", type=int, default=5000, help="Number of pairs to load from dataset"
    )
    parser.add_argument(
        "--experiment", action="store_true", help="Run full experiment suite from config"
    )
    parser.add_argument(
        "--config", help="Path to experiment config (e.g., experiments/minilm_baseline.yaml)"
    )
    parser.add_argument(
        "--output", default="docs/experiment_results.md", help="Output markdown file"
    )
    ...
    if args.experiment:
        ...
        try:
            ...
            logger.info(
                f"Running experiments with {len(config['experiment']['batch_sizes'])} batch sizes and {len(config['experiment']['concurrency_levels'])} concurrency levels"
            )
            logger.info(
                f"Total experiments: {len(config['experiment']['batch_sizes']) * len(config['experiment']['concurrency_levels'])}"
            )
        ...
        if results and config:
            ...
            print("=" * 120)
            print(
                f"{'Batch':<6} {'Conc':<5} {'Pairs':<8} {'Time':<8} {'Lat Avg':<10} {'Lat P95':<10} {'Lat P99':<10} {'TP Avg':<12} {'TP P99':<12}"
            )
            print("-" * 120)
            for r in results:
                if "error" in r:
                    ...
                    if r.get("interrupted"):
                        status_str = "INTRPT"
                    print(
                        f"{r['batch_size']:<6} {r['concurrency']:<5} {status_str:<8} {'N/A':<8} {'N/A':<10} {'N/A':<10} {'N/A':<10} {'N/A':<12} {'N/A':<12}"
                    )
                else:
                    ...
                    if r.get("interrupted"):
                        pairs_str += "*"
                    print(
                        f"{r['batch_size']:<6} {r['concurrency']:<5} {pairs_str:<8} "
        ...
        try:
            ...
            if "error" not in result:
                ...
                print(
                    f"Throughput: avg={result['throughput_avg']:.1f} p/s, p95={result['throughput_p95']:.1f} p/s, p99={result['throughput_p99']:.1f} p/s"
                )
                if result.get("interrupted"):
                    print(
                        f"(Interrupted after {result['num_requests']}/{result.get('requested_requests', 'N/A')} requests)"
                    )
            else:
