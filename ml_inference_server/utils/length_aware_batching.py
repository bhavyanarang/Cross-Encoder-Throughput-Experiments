...
def sort_pairs_by_length(
    ...
    # Estimate lengths
    lengths = [
        estimate_pair_length(q, d, chars_per_token) 
        for q, d in pairs
    ]
...
def bucket_pairs_by_length(
    pairs: List[Tuple[str, str]],
    bucket_boundaries: List[int] = [64, 128, 256, 384],
    chars_per_token: float = 4.0,
...
class LengthAwareBatcher:
    ...
    def __init__(
        ...
        self.sorted_pairs, self.sorted_indices, self.unsort_indices = \
            sort_pairs_by_length(pairs, chars_per_token)
    ...
    def estimate_padding_reduction(self, batch_size: int = 32) -> dict:
        ...
        reduction_pct = ((random_padding - aware_padding) / random_padding * 100) if random_padding > 0 else 0.0
        ...
        return {
            "random_batching": {
                ...
                "padding_pct": round(random_padding / random_total_tokens * 100, 1) if random_total_tokens > 0 else 0.0,
            },
            "length_aware_batching": {
                "total_tokens": aware_total_tokens,
                "real_tokens": aware_real_tokens,
                "padding_tokens": aware_padding,
                "padding_pct": round(aware_padding / aware_total_tokens * 100, 1) if aware_total_tokens > 0 else 0.0,
            },
