...
class ONNXBackend(BaseBackend):
    ...
    def __init__(
        self, model_name: str, device: str = "mps", optimize: bool = True, use_coreml: bool = True
    ):
    ...
    def load_model(self) -> None:
        ...
        self.session = ort.InferenceSession(
            onnx_path, sess_options=sess_options, providers=self.providers
        )

        self._is_loaded = True
        logger.info(
            f"Loaded ONNX cross-encoder on {[p[0] if isinstance(p, tuple) else p for p in self.providers]}"
        )
    ...
    def _get_providers(self, ort) -> list:
        ...
        if self.use_coreml and "CoreMLExecutionProvider" in available:
            ...
            logger.info(
                f"Using CoreML with ALL compute units (CPU+GPU+ANE), batch={self._fixed_batch_size}"
            )
    ...
    def _get_or_export_onnx_model(self) -> str:
        ...
        if self.use_coreml:
            onnx_path = (
                cache_dir / f"{model_safe_name}_cross_encoder_batch{self._fixed_batch_size}.onnx"
            )
        else:
    ...
    def infer_with_timing(self, pairs: list[tuple[str, str]]) -> InferenceResult:
        ...
        try:
            ...
            if self.use_coreml and self._fixed_batch_size:
                ...
                queries, documents = zip(*pairs, strict=False) if pairs else ([], [])
                inputs = self.tokenizer(
                    list(queries),
                    list(documents),
                    return_tensors="np",
    ...
    def _infer_fixed_batch(self, pairs: list) -> np.ndarray:
        ...
        for i in range(0, batch_size, fixed_size):
            ...
            if logits.shape[-1] == 1:
                ...
                all_scores[result_idx : result_idx + chunk_size] = 1.0 / (
                    1.0 + np.exp(-logits[:chunk_size, 1])
                )
