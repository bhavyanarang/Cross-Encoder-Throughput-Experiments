...
class PaddingAnalyzer:
    ...
    def get_stats(self) -> PaddingStats:
        ...
        with self._lock:
            ...
            total_wasted_pct = (
                (self.padded_tokens_total / total_all_tokens * 100) if total_all_tokens > 0 else 0.0
            )
            ...
            return PaddingStats(
                ...
                avg_max_seq_length=int(np.mean(self.max_seq_lengths))
                if self.max_seq_lengths
                else 0,
                avg_avg_seq_length=round(float(np.mean(self.avg_seq_lengths)), 1)
                if self.avg_seq_lengths
                else 0.0,
