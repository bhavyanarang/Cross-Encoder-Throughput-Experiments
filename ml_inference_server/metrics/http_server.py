...
class MetricsServer:
    ...
    def update_history(self) -> None:
        ...
        if is_running and current_count > self.history.last_request_count:
            ...
            padding_pct = padding_analysis.get(
                "last_padding_pct", padding_analysis.get("avg_padding_pct", 0)
            )
            ...
            if instances:
                ...
                if not self.history.instance_names:
                    self.history.instance_names = [
                        inst.get("name", f"inst-{i}") for i, inst in enumerate(instances)
                    ]
