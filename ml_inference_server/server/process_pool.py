...
def _worker_main(
    ...
    try:
        ...
        if max_length:
            logger.info(f"[Worker {worker_id}] Using configured max_length: {max_length} (model default: {model.max_length})")
...
class ModelPool:
    ...
    def get_instance_metrics_summary(self) -> dict:
        ...
        avg_util = sum(inst["utilization_pct"] for inst in instances) / len(instances) if instances else 0.0
    ...
    def get_instance_metrics_history(self) -> dict:
        ...
        return {
            ...
            "request_counts": [self._worker_request_counts.get(i, 0) for i in range(self.num_workers)],
